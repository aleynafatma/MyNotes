...
public class MainActivity extends AppCompatActivity implements  NoteFragment.OnNoteListInteractionListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listenerRegistration = db.collection("notes").orderBy("data", Query.Direction.DESCENDING).addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                if (error != null) {
                    Log.e("Firabase","Error retriveing notes");
                    return;
                }
                notes.clear();
